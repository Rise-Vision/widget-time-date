...
RiseVision.Common.LoggerUtils = (function() {
  ...
  function getEventParams(params, cb) {
    ...
    if (params.event) {
      ...
      if(location.href) {
          var parent = getParameterByName("type", location.href); // templates
          var type = getParameterByName("type", location.href); // legacy presentations
          
          json.type = type ? type : (parent ? getParameterByName("type", parent) : "");
      }
...
RiseVision.Common.Logger = (function(utils) {
  ...
  function log(tableName, params) {
    ...
    if ((!params.type || params.type !=="sharedschedule") && (!params.display_id || params.display_id === "preview" || params.display_id === "display_id" ||
